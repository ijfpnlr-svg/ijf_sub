...
def load_crps_series_losses():
    if not os.path.exists(
        ...
        raise FileNotFoundError(
            f"Missing file:\n{CRPS_SERIES_LOSS_FILE}\n\n"
            "Run the updated simulation reconciliation script first. "
            "It must export per-series and per-time CRPS losses."
        )
...
def write_text_summary(
    ...
    lines.append(
        "SIMULATION SEQUENTIAL STUDENTIZED MCS INTERPRETATION"
    )
...
def write_latex_tables(
    ...
    for (
        ...
        lines.append(
            "    "
            r"\caption{Sequential studentized MCS elimination steps for the "
            f"{target_latex} simulation surface, with "
            rf"$\alpha={alpha}$."
            r"}"
        )
        lines.append(
            f"    \\label{{tab:sim_mcs_steps_{label_suffix}}}"
        )
        lines.append(
            r"\end{table}"
        )
...
def main():
    print(
        "Loading simulation per-series CRPS losses..."
    )
