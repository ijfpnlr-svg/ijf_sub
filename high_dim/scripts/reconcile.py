...
def compute_energy_loss_by_time(
    ...
    This is used for:
        ...
        RelES_method
        =
        mean_t ES_method,t / mean_t ES_base,t
    """
...
def compute_crps_loss_by_time(
    ...
    """
    Compute one average CRPS loss per forecast time step.

    At each time step, CRPS is averaged across all variables.

    This preserves your previous loss export for DM-style scripts.
    """
    observations = np.asarray(
        observations,
        dtype=float,
...
def compute_crps_loss_by_variable_and_time(
    ...
    """
    Compute CRPS for each variable and each forecast time step.
    ...
    This is needed for the paper metric:
        ...
        =
        geometric mean_j [
            mean_t CRPS_method,j,t
            /
            mean_t CRPS_base,j,t
        ]
    """
    observations = np.asarray(
        observations,
...
def append_loss_rows_for_case(
    ...
    """
    Append loss rows for later significance tests.
    ...
    containing:
        - one Energy Score loss per time step
        - one average CRPS loss per time step

    crps_series_rows creates:

        reconciliation_crps_by_series_time.csv

    containing:
        - one CRPS loss per variable and time step

    The second file is required to reproduce the geometric-mean
    relative CRPS used in the paper.
    """
...
def main():
    ...
    for dimension in dimensions:
        ...
        for surface in surfaces:
            ...
            print(
                f"CRPS series loss checkpoint saved to: "
